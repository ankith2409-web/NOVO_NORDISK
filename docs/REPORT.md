...
**Implemented but not live-verified:** the Snowflake, Databricks, Redshift and Athena
connectors. Each authenticates and hands a DB-API cursor to the same adapter DuckDB uses,
and each is unit-tested against fake cursors — driver parameters, per-platform identifier
case folding, SQL dialect and connection lifecycle. None has been run against a real
account, because the network this was built on blocks all four at the policy layer, so the
first live connection is the acceptance test. DuckDB stays the default because it needs no
account, no credentials and no network — which is what lets anyone clone the repository
and run the reconciliation demo in one command.
