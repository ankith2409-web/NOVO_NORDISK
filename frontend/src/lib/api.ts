...
/**
 * The captured run, if this build has one.
 *
 * `import("./snapshot.json")` looks harmless and is not: Vite resolves a
 * static specifier at build time, so the file has to exist for the module to
 * be *parsed*, in every build and every test run. The snapshot is generated by
 * `concordance snapshot` and gitignored, so it does not exist in a fresh
 * clone -- and the effect was that `api.test.ts` could not be collected at
 * all. Sixteen tests silently absent from the suite on any machine that had
 * not run the capture, which is worse than a failure because it looks like
 * fewer tests rather than a broken one.
 *
 * `import.meta.glob` is the Vite-shaped answer: it resolves the pattern
 * against what is on disk, so the map is empty when the file is missing and
 * carries one loader when it is there. No file, no import, no parse error, and
 * a live build behaves exactly as before.
 */
async function loadSnapshot(): Promise<Record<string, unknown> | null> {
  const found = import.meta.glob<{ default: Record<string, unknown> }>(
    "./snapshot.json",
  );
  const load = found["./snapshot.json"];
  if (!load) return null;
