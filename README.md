...
| | Sales & Returns | Supply Chain | Clinical Trial |
|---|---|---|---|
| Format | .pbix | .pbix | TMDL |
| Tables (user) | 16 | 6 | 8 |
| Columns | 86 | 47 | 56 |
| Measures written by an author | 58 | 4 | 24 |
| Figures the report computes on a visual | 5 | 3 | 0 |
| Relationships | 8 | 2 | 10 |
| Hierarchies | 2 | 1 | 4 |
| Graph nodes / edges | 167 / 302 | 63 / 75 | 98 / 157 |
| Unresolved references | 0 | 0 | 0 |
| Unextracted features | 0 | 0 | 0 |
| Requirements (BRD / FRD) | 73 / 89 | 10 / 17 | 39 / 47 |

Two rows there answer a question worth answering, because the numbers look uneven and the
reason is not what it appears.

**Measure counts differ because the models differ, not because reading failed.** Power BI
lets an author drop a bare column onto a chart and choose an aggregation on the tile
instead of writing a measure. Whole reports are built that way — Microsoft's own
AdventureWorks sample carries exactly *one* measure, and every figure a reader sees on its
three pages is an implicit `Sum` declared on a visual. A model like that is not
half-extracted; it genuinely contains one measure.

Those implicit figures are now documented too, which is what the second row counts. Until
they were, a BRD for such a model listed the one authored measure and none of the figures
actually on screen — complete as a description of the *model* and useless as a description
of the *solution*. Each is marked in the document as produced by the report rather than by
a measure, and each is bound to the column it aggregates, so the traceability still holds.

Nothing here is estimated: the table, the column and the aggregation are all stated by the
tile, and an aggregation with no exact DAX equivalent is left out rather than approximated.
