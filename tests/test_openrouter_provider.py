"""The OpenRouter provider's request and response translation.

No network call anywhere here, for the same reason as the Groq provider's
tests: this sandbox's egress policy blocks ``openrouter.ai`` outright
(confirmed via a direct CONNECT, answered with a policy 403), so what can be
verified from here is that the translation is correct against OpenRouter's
documented OpenAI-compatible contract, not that a specific key answers it.
Run one real, free question locally before relying on this for a demo.

The wire format is byte-identical to Groq's -- both are OpenAI-style chat
completions -- so these tests mirror ``test_groq_provider.py`` rather than
inventing new cases for a format that behaves the same way.
"""
...
from concordance.llm.openrouter import (
    DEFAULT_MODEL,
    OpenRouterProvider,
    _to_completion,
    _to_messages,
...
def test_the_request_carries_the_bearer_key_and_attribution_headers() -> None:
    """OpenRouter-specific: the two headers that attribute usage in its
    dashboard. Neither is required to succeed, but a regression that drops the
    Authorization header would fail silently as a 401 far from this test."""
    provider = OpenRouterProvider(api_key="sk-or-v1-test", base_url="http://127.0.0.1:1")
    request = provider._post  # not called; inspect construction instead
    del request
    ...
    def fake_urlopen(req, timeout=None):  # noqa: ANN001
        ...
        raise urllib.error.URLError("not actually calling out")
    ...
    assert "Http-referer" in captured["headers"]  # urllib capitalises only the first segment
...
def test_a_missing_key_fails_fast_with_an_actionable_message(monkeypatch) -> None:
    monkeypatch.setattr("concordance.llm.openrouter._key_from_environment", lambda: None)
...
def test_the_api_key_never_appears_in_a_raised_error() -> None:
    ...
    from concordance.llm.openrouter import _describe, _redact
...
def test_is_registered_as_a_fallback_chain_choice() -> None:
    """Regression: available_providers() must be able to build this provider
    when OPENROUTER_API_KEY is the only key present."""
    import os
